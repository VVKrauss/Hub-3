/*
  # Fix Database Relationship and RLS Policies
  
  1. Changes
    - Fix infinite recursion in sh_users RLS policies
    - Create proper foreign key relationship between sh_event_speakers and sh_speakers
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE tablename = 'sh_users' AND policyname = 'sh_users_select_own_simple'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'sh_users' AND policyname = 'sh_users_select_admin_simple'
    ) THEN
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'sh_site_settings' AND policyname = 'sh_site_settings_admin_access'
    ) THEN
