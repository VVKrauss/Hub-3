...
-- Handle the foreign key relationship carefully
DO $$
BEGIN
  ...
  IF NOT EXISTS (
    ...
    -- Clean up invalid references before adding the constraint
    DELETE FROM sh_event_speakers
    WHERE speaker_id IS NOT NULL AND NOT EXISTS (
      SELECT 1 FROM sh_speakers WHERE sh_speakers.id = sh_event_speakers.speaker_id
    );
