...
const AdminLayout = () => {
  ...
  const navItems = [
    ...
    { to: '/admin/courses', icon: BookOpen, label: 'Курсы', shortLabel: 'Курсы' },
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-dark-900">
      ...
      `}>
          ...
          <div className="space-y-1 px-2">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <Icon className={`h-5 w-5 ${isSidebarCollapsed && !isMobile ? 'mx-auto' : ''}`} />
                  {(!isSidebarCollapsed || isMobile) && (
        ...
        <div className="border-t border-gray-200 dark:border-dark-700 p-2">
          {/* Profile section */}
          <div className={`flex items-center gap-3 px-3 py-2 ${isSidebarCollapsed && !isMobile ? 'justify-center' : ''}`}>
          ...
          {/* Action buttons */}
          <div className={`flex gap-2 px-2 mt-2 ${isSidebarCollapsed && !isMobile ? 'flex-col' : ''}`}>
            <button
              onClick={() => window.open('/', '_blank')}
              className="flex items-center justify-center gap-2 px-3 py-2 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-dark-700 rounded-lg transition-colors flex-1"
              title="Посмотреть сайт"
            >
              <ExternalLink className="h-4 w-4" />
              {(!isSidebarCollapsed || isMobile) && <span>Сайт</span>}
            </button>
            
            <button
              onClick={handleLogout}
              className="flex items-center justify-center gap-2 px-3 py-2 text-sm text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors flex-1"
              title="Выйти"
            >
              <LogOut className="h-4 w-4" />
              {(!isSidebarCollapsed || isMobile) && <span>Выйти</span>}
            </button>
          </div>
        </div>
      </aside>
...
export default AdminLayout;// src/components/admin/AdminLayout.tsx
import { Outlet, NavLink, useNavigate, useLocation } from 'react-router-dom';
import { createClient } from '@supabase/supabase-js';
import { useState, useEffect } from 'react';
import { 
