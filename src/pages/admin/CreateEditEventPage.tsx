      ...
      if (!id || slugManuallyEdited) {
        ...
        if (existingEvent && existingEvent.length > 0) {
          ...
          if (existingId !== id) {
            ...
            return;
          // Test 10: Check if this is an RLS (Row Level Security) issue
          ...
          // Try to read the event again to make sure we can access it
          const { data: rereadEvent, error: rereadError } = await supabase
            .from('sh_events')
            .select('*')
            .eq('id', id)
            .single();
            
          console.log('Can we still read the event?', { 
            success: !rereadError, 
            error: rereadError?.code,
            hasData: !!rereadEvent 
          });
          ...
          // Test 8: Try using SQL directly via supabase.sql() if available
          console.log('Trying direct SQL approach...');
          
          try {
            // Use the Supabase SQL interface if available
            const sqlQuery = `
              UPDATE sh_events 
              SET 
                event_type = $1,
                ...
                updated_at = NOW()
              WHERE id = $3
              RETURNING event_type, status, updated_at
            `;
            
            // Try using .rpc with a custom function that bypasses triggers
            const { data: sqlResult, error: sqlError } = await supabase
              .rpc('direct_update_sh_event', {
                p_event_id: id,
                p_event_type: eventData.event_type,
                p_status: eventData.status
              });
              
            console.log('Direct SQL via RPC result:', { sqlResult, sqlError });
            
            // If RPC doesn't exist, we'll get an error - that's expected
            
          } catch (sqlErr) {
            console.log('Direct SQL approach not available:', sqlErr);
          }
          
          // Test 9: Try the most basic possible update to see what happens
          console.log('Trying minimal single-field updates...');
          
          // Test just updating the title to see if ANY field can be changed
          ...
          console.log('Title-only update error:', titleOnlyError);
          ...
          if (!titleOnlyError) {
            const { data: titleCheck } = await supabase
              ...
              .eq('id', id)
              .single();
            console.log('Title after update:', titleCheck?.title);
            console.log('Title actually changed:', titleCheck?.title === testTitle);
          }
          
          // Test 7: Try creating a completely new event to see if the same issue occurs
          console.log('Testing with new event creation...');
          const testEventData = {
          ...
          console.log('New event creation result:', { newEvent, newEventError });
          
          if (newEvent) {
            // Try updating the new event
            const { error: newUpdateError } = await supabase
              .from('sh_events')
              .update({ event_type: 'workshop', status: 'active' })
              .eq('id', newEvent.id);
              
            console.log('New event update result:', newUpdateError);
            
            const { data: newEventCheck } = await supabase
              .from('sh_events')
              .select('event_type, status')
              .eq('id', newEvent.id)
              .single();
              
            console.log('New event after update:', newEventCheck);
            
            // Clean up test event
            await supabase.from('sh_events').delete().eq('id', newEvent.id);
          }
          
          // Test 6: Check if the issue is date-based (maybe status is auto-calculated from dates)
          console.log('Testing if status is date-based...');
          
          // Try setting a future date and see if status can change
          ...
          const { error: futureDateError } = await supabase
            ...
            .update({ 
              start_at: futureDate.toISOString(),
              end_at: new Date(futureDate.getTime() + 2 * 60 * 60 * 1000).toISOString() // +2 hours
            })
            .eq('id', id);
            
          console.log('Future date update result:', futureDateError);
          
          if (!futureDateError) {
            // Now try setting status again
            const { error: statusAfterDateError } = await supabase
              .from('sh_events')
              .update({ status: 'active' })
              .eq('id', id);
              
            console.log('Status update after future date:', statusAfterDateError);
            
            const { data: afterDateCheck } = await supabase
              .from('sh_events')
              .select('status, start_at')
              .eq('id', id)
              .single();
              
            console.log('Status after future date set:', afterDateCheck);
          }
        }
      }
